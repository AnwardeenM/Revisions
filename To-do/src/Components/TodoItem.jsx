     ...
     <div className='flex items-center justify-between gap-2'>
      {
        isEditing ? (
          ...
          onChange={(e) => setEditText(e.target.value)
            ...
            <label className={`hover:bg-slate-100 flex-1 p-2 rounded-md cursor-pointer select-none ${isComplete ? "line-through text-slate-600" : "" }`}
        ...
        <div className='flex gap-2'>
          {isEditing ? (
            <button
            className='text-white rounded px-2 py-1 rounded-md bg-green-500 hover:bg-green-600 text-xs' onClick={handleSave}
            onClick = {handleSave}
            >
              Save
              </button>            
