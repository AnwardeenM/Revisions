...
export const TodoItem = ({text,isComplete,id,toggleTodo}) => {
  return (
    <>
     <div className='flex items-center justify-between gap-2'>
        <label className={`hover:bg-slate-100 flex-1 p-2 rounded-md cursor-pointer select-none ${isComplete ? "line-through text-slate-600" : "" }`} onClick={()=>{
          toggleTodo(id)
        }}>{text}</label>
        <div>
